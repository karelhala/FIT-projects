...
$table = "Plany p LEFT JOIN Letadlo l ON p.ID_letadlo = l.ID LEFT JOIN Test tt ON p.ID_test = tt.ID LEFT JOIN Ma_provest m ON m.ID_Plany = p.ID";
$where = "WHERE Hotovo = 0 GROUP BY p.ID";
...
if ($query) {
	$where = " WHERE $qtype LIKE '%".$query."%' AND Hotovo = 0";
}

$sql = "SELECT p.ID, p.Datum_zadani,  l.typ,  l.seriovec, tt.popis AS test_pop, m.login_technika  FROM $table $where $sort $limit";
$result = runSQL($sql);

$total = countRec("p.ID","$table WHERE Hotovo = 0");
...
while ($row = mysql_fetch_assoc($result)) {
		...
		$prirazeno = ($row['login_technika'] == null) ? "Ne" : "Ano";
