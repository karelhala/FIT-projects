 ...
 */
static inline int udt_init(in_port_t local_port,char* interface)
{
	ifreq ifr;
	int sock;
	...
	//nacteni jmena zarizeni a nacteni jeho IP adresy
 ...
 */
static inline int udt_recv(int udt, void *buff, size_t nbytes, in_addr_t *addr, in_port_t *port,struct sockaddr_in* pom)
{
	struct stat info;
	if (fstat(udt, &info) != 0) {
